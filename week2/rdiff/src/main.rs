...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    for i in 0..m {
        for j in 0..n {
            // let mut result;
            if seq1[i] == seq2[j] {
                if let Err(result) = grid.set(i + 1, j + 1, grid.get(i, j).unwrap() + 1) {
                    panic!("{}", result);
                }
            } else {
                if let Err(result) = grid.set(
                    i + 1,
                    j + 1,
                    grid.get(i, j + 1).unwrap().max(grid.get(i + 1, j).unwrap()),
                ) {
                    panic!("{}", result);
                }
            }
...
fn main() {
    ...
    let lines1 = read_file_lines(&String::from(filename1)).expect("Error reading file1");
    let lines2 = read_file_lines(&String::from(filename2)).expect("Error reading file2");
